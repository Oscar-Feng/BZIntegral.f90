...
! Testing Quad3DRuleΘδ and Quad3DRuleΘΘδ.
!                fₖ (1- fₖ₊ₚ)      fₖ (1- fₖ₋ₚ)
! χ(p,ω) = ∫d³k ------------- +  -------------
!                ω-ϵₖ+ϵₖ₊ₚ+iη      -ω-ϵₖ+ϵₖ₋ₚ-iη
  ...
  call CPU_TIME(t1)
  call Quad3DRuleThetaDelta(Emesh,eF,D1mesh,Wmesh_1,iter)
  call Quad3DRuleThetaThetaDelta(mnsEmesh,Eplusmesh,D1mesh, Wmesh_2,iter)
  call Quad3DRuleThetaDelta(Emesh,eF,D2mesh,Wmesh_3,iter)
  call Quad3DRuleThetaThetaDelta(mnsEmesh,Emnusmesh,D2mesh, Wmesh_4,iter)
  call CPU_TIME(t2)

  Wmesh=Wmesh_1-Wmesh_2-Wmesh_3+Wmesh_4

  print*, "q=", q, "v=", v, "iter=", iter
  print*, "Lindhard3D_ImX, result=", sum(Wmesh)*vol*PI
